const open = require('open');
const logger = require('../../config/utils/logger')('Create repo');

module.exports = () => {
  logger.error(`Your personal access token was rejected, maybe because it has expired.

Create a new one (with "repo" permissions and SSO enabled) and then be sure to update your password in Keychain Access.

Check your browser for instructions.
`);
  open('https://github.com/settings/tokens', { background: true });
  open('https://help.github.com/en/github/using-git/updating-credentials-from-the-osx-keychain#updating-your-credentials-via-keychain-access', { background: true });
  process.exit(0);
};
